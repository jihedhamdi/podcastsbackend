        ...
        <section class="content">
            ...
            <div class="box">
                <div class="box-header with-border">
                    ...
                    <a class='col-lg-offset-5 btn btn-success' href="{{ route('gestion-page-informative.create') }}">Ajouter
                        une Page Informaives</a>
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody id="tablecontents">
                                    @foreach ($pageinformatives as $pageinformative)
                                        <tr class="row1" data-id="{{ $pageinformative->id }}">
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td>{{ $pageinformative->titre }}</td>
                                            <td>{{ $pageinformative->slug }}</td>
                                            <td class="pl-3"><i class="fa fa-sort">  {{ $pageinformative->Order }}</td>
                                            <td><a href="{{ route('gestion-page-informative.edit', $pageinformative->id) }}"><span
                                                        class="glyphicon glyphicon-edit"></span></a>
                                                <form id="delete-form-{{ $pageinformative->id }}" method="post"
                                                    action="{{ route('gestion-page-informative.destroy', $pageinformative->id) }}"
                                                    style="display: none">
                                                    {{ csrf_field() }}
                                                    {{ method_field('DELETE') }}
                                                </form>
        ...
        $(function() {
            ...
            function sendOrderToServer() {
                ...
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "{{ route('gestion-page-informative.updateOrder') }}",
